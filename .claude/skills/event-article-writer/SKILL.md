...
- **Section writing guidance:**
  ...
  - `## 内容`: 最初に 1〜2文の prose で全体の魅力・性格を説明し、その後にソースページで紹介されている具体的な展示物・コラボメニュー/グッズ・演出・キャンペーン内容・特典などを、ソースの記述に沿ってできるだけ詳細に箇条書きで列挙する。要約しすぎず、ソースが挙げている固有名詞（商品名・メニュー名・演者名など）を落とさない。prose なしで箇条書きだけで始めない。
