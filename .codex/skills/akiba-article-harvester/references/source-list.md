...
Most sources below (except X.com) are fetched and parsed in one pass by `../scripts/harvest.py list all` — see [SKILL.md](../SKILL.md#harvest-script). Read that script's output from a redirected file rather than fetching these pages one at a time.
